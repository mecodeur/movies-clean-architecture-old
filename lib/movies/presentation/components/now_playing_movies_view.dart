  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<MoviesBloc, MoviesState>(
      builder: (context, state) {
        print('>>>>>>>>>>>>>>>< BlocBuilder NowPlaying : ${state} ');
        if(state is NowPlayingMoviesSuccess){
          ...
          return Container(
              height: 200,
              child: Center(child: Text('State: ${state.errMessage}', style: TextStyle(color: Colors.white),)));
        }
        else if(state is NowPlayingMoviesLoading){return SizedBox(
            height: 400,
